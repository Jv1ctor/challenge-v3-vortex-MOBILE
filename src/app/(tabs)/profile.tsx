...
export default function Profile() {
  ...
  const { loading, data, isError, fetchGetProfile } = useOperator(user?.token ?? null);
        ...
        <View className="mt-10 items-center gap-10 px-10">
          <View className="w-full items-center rounded-xl bg-card p-5" style={styles.shadow}>
            ...
            <View>
              ...
              <Text className="text-center text-xl font-bold text-accentForeground">
                {data?.factory_name}
              </Text>
          ...
          <View className="flex-row items-center gap-10">
            ...
            <View className="gap-3 rounded-xl bg-card p-5" style={styles.shadow}>
              ...
              <Text className="text-xl font-semibold text-foreground">
                {data?.last_registry_at ? data.last_registry_at : 'Sem registro'}
              </Text>
            </View>
          </View>

          <Button className="w-2/3 bg-red-800" onClick={handleLogout}>
            Sair
          </Button>
        </View>
      </ScrollView>
